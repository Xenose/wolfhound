...
#else
    i64 _jmp_index = 0;
    i64 _jmp_error[JMP_MAX] = { 0 };
    sigjmp_buf _jmp_buffers[JMP_MAX] = { 0 };
    struct sigaction _old_sigaction[JMP_MAX] = { 0 };
#endif
...
#else
    // ON TinyCC THIS IS NOT SUPPORTED!
    _wh_try_info_s* _jmp_info = nullptr;
#endif
