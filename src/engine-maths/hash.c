...
u32 _wh_hash_xx32(_wh_hash_xx32_params params) {
	...
	// step 2
	for (; 16 <= bytes; bytes -= 16, index += 4) {
		a1 = a1 + (((u32*)params.buffer.ptr)[index] * p2);
		a1 = (a1 << 13) | (a1 >> (32 - 13)); // bit rotation
		a1 = a1 * p1;

		a2 = a2 + (((u32*)params.buffer.ptr)[index + 1] * p2);
		a2 = (a2 << 13) | (a2 >> (32 - 13)); // bit rotation
		a2 = a2 * p1;

		a3 = a3 + (((u32*)params.buffer.ptr)[index + 2] * p2);
		a3 = (a3 << 13) | (a3 >> (32 - 13)); // bit rotation
		a3 = a3 * p1;

		a4 = a4 + (((u32*)params.buffer.ptr)[index + 3] * p2);
...
go_step_4:
	...
	for (;4 <= bytes; bytes -= 4, index += 4) {
		a_out += (((u32*)params.buffer.ptr)[index] * p3);
